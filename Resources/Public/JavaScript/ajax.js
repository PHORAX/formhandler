  ...
  $.fn.formhandler = function (options) {
    var defaults = {
      ...
      fieldValidationBindings: function () {
        ...
        var shortName = name
          .replace(settings.formValuesPrefix, '')
          .replace('[', '')
          .replace(']', '');
        ...
        var url =
          ...
          settings.lang;
        var postData =
          formhandlerDiv.find('form').serialize() +
          '&' +
          formhandlerDiv.find(settings.submitButtonSelector).attr('name') +
          '=submit';
          ...
          success: function (data, textStatus) {
            ...
            if (settings.autoDisableSubmitButton) {
              ...
              if (valid) {
                ...
                button
                  .removeClass(settings.validationStatusClasses.invalid)
                  .addClass(settings.validationStatusClasses.valid);
              } else {
                button.attr('disabled', 'disabled');
                button
                  .removeClass(settings.validationStatusClasses.valid)
                  .addClass(settings.validationStatusClasses.invalid);
              }
            }
          },
        });
    ...
    for (index = 0; index < settings.validateFields.length; ++index) {
      ...
      formhandlerDiv.on(
        settings.defaultValidationEvent,
        "input[name^='" +
          fieldName +
          "'],select[name^='" +
          fieldName +
          ...
          fieldName +
          "']",
        settings.fieldValidationBindings
      );
    }
  };
})(jQuery);
