...
Event.observe(window, 'load', function () {
  ...
  var templateFileName =
    'data[tt_content][' + uid + '][pi_flexform][data][sDEF][lDEF][template_file][vDEF]_list';
  var langFileName =
    'data[tt_content][' + uid + '][pi_flexform][data][sDEF][lDEF][lang_file][vDEF]_list';
  var predefinedName =
    'data[tt_content][' + uid + '][pi_flexform][data][sDEF][lDEF][predefined][vDEF]';
  var requiredFieldsName =
    'data[tt_content][' + uid + '][pi_flexform][data][sMISC][lDEF][required_fields][vDEF]_hr';
  var templateFileHiddenName =
    'data[tt_content][' + uid + '][pi_flexform][data][sDEF][lDEF][template_file][vDEF]';
  var langFileHiddenName =
    'data[tt_content][' + uid + '][pi_flexform][data][sDEF][lDEF][lang_file][vDEF]';
  var requiredFieldsHiddenName =
    'data[tt_content][' + uid + '][pi_flexform][data][sMISC][lDEF][required_fields][vDEF]';
  ...
  if (newRecord) {
    ...
    for (var index = 0; index < options.length; index++) {
      if (options[index].value == 'default.') {
        ...
        $(templateFile).insert(
          '<option value="' + templateFileDefault + '">' + templateFileDefault + '</option>'
        );
        $(langFile).insert(
          '<option value="' + langFileDefault + '">' + langFileDefault + '</option>'
        );
        break;
      }
    }
  ...
  Event.observe(predefined, 'change', function () {
    if (this.value != 'default.') {
      if (
        typeof templateFile.options[0] != 'undefined' &&
        templateFile.options[0].value == templateFileDefault
      ) {
        templateFile.removeChild(templateFile.options[0]);
        templateFileHidden.value = '';
      }
      if (
        typeof langFile.options[0] != 'undefined' &&
        langFile.options[0].value == langFileDefault
      ) {
